...
## Purpose
Manage the team roster (employment + leave-balance record), organise roles into groups, define shift-coverage templates, review/approve leave requests, and run weekly payroll - for the **active building** (`wesley`, constant this phase). Five tabs: **Team**, **Roles & groups**, **Shift templates**, **Leave requests**, **Payroll**.
...
| Team | `team-tab` | Directory table: avatar+name+"Since {start}", **roles** (one or more chips), contract pill + weekly hours, **Visa** (type + colored expiry chip: red expired / amber ≤60d / green), **Leave left (A / S)** column - annual over sick, each `remaining / entitled` with its own mini bar (terracotta when ≤2 days left else sage), phone, status dot, edit/delete actions. **Client search** (name/role/contract/visa/phone/status) + **pagination** (10/page): search box + "X–Y of Z" count above the table, numbered pager (‹ 1 2 … ›) below; searching resets to page 1 |
| Roles & groups | `roles-groups-tab` | **Role registry CRUD** + **ordered groups** that band/sort the roster. Groups list (reorder ↑/↓, rename via add-form, delete → members fall back to Unassigned) with member-role chips (**reorder ↑/↓ within the group** - e.g. Registered Nurse above Carer - + removable) + "+ Add role…" select + staff-count pill; an **Unassigned roles** bucket (assign-to-group select); an **All roles** list (color swatch, group, staff count, **inline Rename** - cascades the new name across staff + shift templates, blocked on a duplicate - + delete, blocked while assigned) + "Add role" form. Admin only |
...
| Payroll | `payroll-tab` | **Admin-only weekly payroll.** Editable per-role **hourly rates** (chips, persisted via `saveRoleRate`) recompute wages live. Paid hours come from the selected week's roster grid (`getPayrollHours`); each person's **gross = paid hours × their primary role's rate**. Week navigator (`‹ ›`, `?week=`) + week badge; 4 KPIs (weekly wage bill / paid hours / rostered staff / avg hours per person); a per-**building** table (Staff · Role · Shifts · Hours · Rate/hr · Gross) with a subtotal row; a navy total-wage-bill footer. No header action button. |
| Leave requests | `leave-tab` | **Two columns.** Left - requests list (`{pending} pending` pill): avatar + name·type, `dates · N shifts · note` line, **Approve/Decline** on `Pending` rows only (decided rows show a status pill), plus a **remove** button on every row. Right - **Leave balance** card: per staff member, an Annual and a Sick row, each `remaining of entitled left` + progress bar (terracotta ≤2 / amber ≤5 / sage). A bucket with no entitlement set reads "Chưa đặt hạn mức" |
...
## Data flow (Supabase)
RSC `page.tsx` calls `src/lib/data/staff.ts` (`getStaff`, `getShiftTemplates`, `getLeaveRequests`) **and `src/lib/data/roles.ts` (`getRoles`, `getRoleGroups`)**, passing the results into `<StaffView>` as props - no client-side fetching for initial load. **For payroll it also reads `?week=` (default current week, same `weekStartOf`/`getRosterDays` helpers as the roster) and calls `src/lib/data/payroll.ts` `getPayrollHours(weekStart, weekEnd)` → per-staff `{hours, shiftCount}` summed from `roster_shifts` × `shift_templates.paid_hours`; `?staffTab=payroll` keeps the payroll tab active across week navigation.** Per-role hourly rates live on `staff_roles.hourly_rate` (migration `0012_role_hourly_rate.sql`; surfaced via `getRoles` → `RoleDef.hourlyRate`) and are edited through the `saveRoleRate(name, rate)` action in `roles.ts`. Role/group writes go through `src/lib/actions/roles.ts` (`saveRole`, `deleteRole`, `renameRole` - via the `rename_role` RPC, cascades the new name across `staff.role`/`staff.roles[]`/`shift_templates.role` in one transaction; migration `0011_rename_role.sql`, `moveRole` - reorder a role within its group, `assignRoleToGroup`, `saveGroup`, `deleteGroup`, `moveGroup`) - each `revalidatePath` both `/portal/staff` and `/portal/roster` (roles/groups drive the roster banding too). `deleteRole` is blocked while any staffer still holds the role. Other writes go through Server Actions in `src/lib/actions/staff.ts`:

- `saveStaff` / `deleteStaff` - upsert/delete `staff` profile fields (name, `roles text[]` + `role` = primary/`roles[0]`, contract → derived `hours`, phone, `visa_type`, `visa_expiry`, **`roster_group_id`** = optional roster-band override, null unless roles span >1 group; migration `0009_staff_roster_group.sql`). At least one role required. `visa_expiry` is forced null for `NZ Citizen`/`Permanent Resident`. Leave **entitlements** (`annual`, `sick`) are editable on the form and default to 0 when left blank; the **consumed** counters (`taken`, `sick_taken`) are never written here - only `approve_leave` moves those, so re-saving a profile can't clobber a balance. Edit also writes `status` from the "Đang làm việc" checkbox (`Active` / `Inactive`); new rows are `Active` with all four leave numbers at 0. **Requires migrations `0004_staff_visa.sql`** (visa columns) **and `0005_staff_multi_role.sql`** (adds `roles text[]`, backfills from `role`).
...
- Team tab: local `query` (search) + `page` (pagination, 10/page), clamped to the filtered result count.
...
## Definition of Done
All three tabs render live Supabase data; team add/edit/delete persist (profile fields only - balances untouched); shift-template add/edit/delete persist; leave request submission persists as Pending; approve debits the counter matching the request type atomically via RPC (`taken` for Annual, `sick_taken` for Sick, nothing for Shift swap); decline updates status only; `tsc`/`lint`/`build` clean.
...
## Leave balances: annual and sick are separate (2026-07-27)

`staff` originally carried one entitlement (`annual`) and one consumed counter (`taken`), and `approve_leave` debited `taken` for **both** Annual and Sick leave — so a sick day silently ate the annual allowance. Migration `0025_staff_leave_balances.sql` adds `sick` / `sick_taken` and rewrites the RPC to debit by request type. Existing `taken` values keep meaning annual days taken; there were no approved requests on record to re-attribute.

The same migration drops the implicit 20-day allowance: `annual` and `taken` now default to 0, and the form's Annual / Sick inputs are the only thing that sets an entitlement. Someone with no number entered has no allowance rather than an inherited one.

Verified by `scripts/db/verify-staff-leave-balances.mts`, which creates and deletes a throwaway staff row: sick leave never touches annual, a shift swap consumes neither, approving twice cannot double-charge, and a new row starts at 0.

## "On leave" is derived, "Inactive" is stored (2026-07-27)

Two different things live in `staff.status`, and only one of them is real data:
