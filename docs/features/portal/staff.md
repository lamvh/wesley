...
| Team | `team-tab` | Directory table: avatar+name+"Since {start}", **roles** (one or more chips), contract pill + weekly hours, **Visa** (type + colored expiry chip: red expired / amber ≤60d / green), **Leave left** column (`remaining/annual` + mini bar, terracotta when ≤2 days else sage), phone, status dot, edit/delete actions. **Client search** (name/role/contract/visa/phone/status) + **pagination** (10/page): search box + "X–Y of Z" count above the table, numbered pager (‹ 1 2 … ›) below; searching resets to page 1 |
