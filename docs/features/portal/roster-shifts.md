...
- **Shift vocabulary + scaffold:** `getShiftDefs()`, `getShiftLegend()`, `getRosterDays(weekStart)`, `dailyTotals()`, `totalShifts()`, `rosterWeekTitle()`, week helpers (`weekStartOf`, `toISODate`, `parseISODate`, `shiftWeek`), `SHIFT_ORDER` (from `roster-schedule.ts` — shift-type reference data only).
