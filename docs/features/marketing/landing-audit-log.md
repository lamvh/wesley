...
Design files - baseline 2026-07-17:

| Design file | Checksum | Modified | Authoritative |
|-------------|----------|----------|---------------|
| `victoria-at-mt-eden.dc.html` | `614ae17572f2` | 2026-07-14 14:06 | ✅ newest - port from this |
...
### Open drift (design ahead of code) - not yet ported

- **Header - login buttons → single "Portal".** Authoritative design (`victoria-at-mt-eden.dc.html`) collapses the two header buttons "Family login" + "Staff portal" into one **"Portal"** button. Code `site-nav.tsx` still renders both (`/login?as=family` + `/login`). **Status: OPEN.**
- **Footer - "Access" column → "Portal".** Same design collapses the footer Access column's "Family login" + "Staff portal" into one **"Portal"** link (Access = Portal + Contact). Code `site-footer.tsx` still lists both login links. **Status: OPEN.**
- **Not drift (verified):** footer **"Our rooms"** column is unchanged in the newest design (VIP suites / Premium suites / Normal rooms / Rest-home care) and matches code - it was **not** removed. Footer "Our home" column also matches.
