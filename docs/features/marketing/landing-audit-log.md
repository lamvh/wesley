...
- **2026-07-18 (rev 6)** - all pages - **Website CMS wired in.** Marketing copy now comes from `getSiteContent()` (Supabase `site_content` override deep-merged over `site-content-defaults.ts`), editable at `/portal/website`. All 6 pages made async; hardcoded copy moved to props; `marketing-content.ts` trimmed to job roles; `footer.blurb` via `layout.tsx`. New checksums: Home `2cc8cc0d44c6`, Our rooms `d3846439e67f`, Life here `bab3b7259f2a`, Our home `0c444982f9cc`, Careers `c779b2e53a63`, Contact `a4dfc33f3d6e`, Footer `a696752f072c`. Defaults also carry design v1.2 copy tweaks (Standard room, Rest-home-care wings). tsc + lint clean.
