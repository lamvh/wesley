...
- **Render:** `src/components/marketing/today-board.tsx` — client component; renders the live `HH:MM` status bar (clock ticks every 15s; full date + sheet date from the device clock) above the shared `DutySheetDocument` (`src/components/portal/roster/duty-sheet-document.tsx`), which is scaled to fill the rest of the screen (see "One layout, scaled to fit"). The sheet — masthead, building header, on-call strip, role bands, Kitchen band, footer date — is the single component the duty-export preview also renders.
...
- **One layout, scaled to fit (2026-08-03):** the board renders the A4 sheet at its native 794px metrics on every device and scales the whole document with a CSS transform so it fits the space under the status bar — the page is `h-dvh`, `overflow-hidden`, so a tablet shows the full roster at once with no scrolling and no cropping. Scale = `min(availableWidth / 794, availableHeight / sheetHeight)`, recomputed by a `ResizeObserver` on both the available area and the sheet (its `offsetHeight` is layout height, untouched by the transform, so measuring cannot feed back into itself); height is measured rather than assumed at 1123px because a long roster can push the document past the A4 minimum. The sheet is hidden until the first measurement so the board never flashes at 1:1.
  - This **replaced the earlier `compact` prop** (2026-07-22 → 2026-08-03): a fluid, smaller-type variant rendered below 640px, which meant `/today` on a small screen showed a *different* document from the printed template. Scaling keeps the two identical at every width — the requirement is "board = printout", so the sheet has no viewport-dependent variant left. Trade-off: on a phone (~390px) the sheet scales to ~0.45, so staff lines render around 7px — legible as a shape, not as reading text. `/today` is a reception-iPad board, not a phone page; note that a phone visitor gets a miniature.
