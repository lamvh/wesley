...
function DutyLine({ time, name }: DutyRow) {
  return (
    <div className="flex items-baseline gap-4 py-[3px]">
      ...
      <span className="text-[15px] font-semibold text-duty-ink">
        {name.toUpperCase()}
      </span>
    </div>
  );
...
//
// Fixed pixel metrics on purpose: this is one document rendered identically by
// the public reception board (/today) and the roster export print preview, so
// both stay pixel-identical to the printout. Surfaces that cannot spare 794px
// scale the whole sheet down (the board does) rather than reflow it — no
// viewport-dependent variant lives here.
